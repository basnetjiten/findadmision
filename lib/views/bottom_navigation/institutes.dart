...
class _InstitutePageState extends State<InstitutePage> {
  ...
  UnivFilterBloc _univFilterBloc;
  ...
  void initState() {
    ...
    _univFilterBloc = getIt<UnivFilterBloc>();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocListener<SaveInstBloc,SaveInstState>(
        cubit: _saveInstBloc,
        listener: (context,state){
          if(state is InstSaved){
            AppWidgetHelper.showSnackBar("Institute saved", context,AppTheme.checkBoxCheckedColor);
          }
          if(state is InstDeleted){
            AppWidgetHelper.showSnackBar("Institute deleted", context,AppTheme.officeBGColor);
          }
        },
        child: Container(
                  ...
                  BlocBuilder<UnivFilterBloc, UnivFilterState>(
                    ...
                    builder: (context, state) {
                      ...
                      if (state is UnivDataFetched) {
                        ...
                        initialCollegeCards = List.generate(
                            univFilterResponse.match.length,
                            (_) => CollegeListCard(
                              favInstBloc:_saveInstBloc,
                                univMatch: univFilterResponse.match[_random
                                    .nextInt(univFilterResponse.match.length)]));
                        return (filteredCollegeCards.isEmpty)
                            ? Column(
                                key: ValueKey<dynamic>(initialCollegeCards),
                                children: [
                                  Text(
                                  ...
                                  ...initialCollegeCards.map((element) {
                                    var index =
                                        initialCollegeCards.indexOf(element);
                                    if (index <= 9) {
                                      return element;
                                    } else {
                                      return SizedBox();
                            ...
                            : isLoading
                                ...
                                : Column(
                                    key: ValueKey<dynamic>(filteredCollegeCards),
                                    children: [
                                      Text(
                                      ...
                                      ...filteredCollegeCards.map((element) {
                                        // get index
                                        //print("ITEM FILTERED",+element.)
                                        var index =
                                            filteredCollegeCards.indexOf(element);
      ...
      univFilterResponse.match.forEach((element) {
        filterDto.feeRange.forEach((feeRangeValue) {
          ...
          if (feeRangeValue.start * 1000 >= element.budgetstart &&
            ...
            setState(() {
              tempCard.add(CollegeListCard(favInstBloc:_saveInstBloc,univMatch: element));
            });
    ...
    univFilterResponse.match.forEach((element) {
      selectedHomeDegree.forEach((degreeFilter) {
        element.degreelist.forEach((degreeIds) {
          if (degreeIds == degreeFilter.degreeId) {
            setState(() {
              tempCourseCardList.add(CollegeListCard(favInstBloc:_saveInstBloc,univMatch: element));
            });
          }
        });
      });
